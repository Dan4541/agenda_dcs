...
class ContactsController extends Controller
{
    ...
    public function store_contact(Request $request){
        ...
        $contact->photo = $request->contact_photo;
        $contact->id_user = Auth::user()->id_user;
        $numbers = collect($request->number);
        $emails = collect($request->email);
        $temp = Contact::latest('id_contact')->first();
        $new_id = ( isset($temp) ? ($temp->id_contact + 1) : 1);        
                

        $numbers->each(function($i) use($new_id){
            PhonesController::store_phone($i, $new_id);
        });        

        $emails->each(function($a) use($new_id){
            EmailsController::store_email($a, $new_id);
        });        
